...
`/room?roomId=<id>&role=owner|participant` 쿼리 파라미터로 방장과 참가자에게 서로 다른 UI를 제공합니다.
...
Amplify 이전 및 WebSocket 별도 도메인 직결 전략은 아래 문서를 참고합니다.

- `docs/AMPLIFY_WEBSOCKET_DIRECT_CONNECT.md`
- Amplify 정적 export 배포는 `NEXT_BUILD_OUTPUT=export` 빌드 환경 변수로 활성화할 수 있습니다.
